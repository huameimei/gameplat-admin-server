...
import com.gameplat.admin.enums.TrueFalseEnum;
...
public class SysUserController {
  ...
  public Result getUserTokenInfo(
    ...
    if (Objects.equals(TrueFalseEnum.YES.getValue(), limit.getVCode())) {
      ...
      if (StringUtil.isBlank(adminLoginDto.getValiCode())
          || !adminLoginDto.getValiCode().equalsIgnoreCase(checkCode)) {
    ...
    if (Objects.equals(TrueFalseEnum.YES.getValue(), limit.getOpenGoogleAuth())) {
      ...
      GoogleConfig googleConfig =
          sysDictDataService.getDictData(DictTypeEnum.GOOGLE_CONFIG.getValue(), GoogleConfig.class);
    ...
    try {
      ...
      HttpUtils.setSessionCookieNotDomain(
          request, response, Constants.ADMIN_TOKEN_NAME, loginAppUser.getToken());
    } catch (Exception e) {
  ...
  @ResponseBody
  public void logout(
      @Session Long adminId, HttpServletRequest request, HttpServletResponse response)
      throws Exception {
