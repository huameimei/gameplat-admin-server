...
public interface MemberInfoMapper extends BaseMapper<MemberInfo> {
     ...
     * @return
     */
    @Select("select mi.rebate from member_info as mi inner join member as m on mi.member_id = m.id where m.account = #{account}")
     ...
     * @return
     */
    @Select("select ifnull(max(mi.rebate),0) from member_info as mi inner join member as m on mi.member_id = m.id where m.parent_name = #{account}")
