...
public class ChatSideMenuServiceImpl implements ChatSideMenuService {
  ...
  public List<ChatSideMenuVO> queryAllSideMenu() {
    ...
    if (StringUtils.isNoneBlank(setting.getDictValue()) && setting.getDictValue() != "[]") {
      ...
      List<String> menuNames =
          settingMenuList.stream().map(ChatSideMenuVO::getName).collect(Collectors.toList());
      List<String> initNames =
          initMenu.stream().map(ChatSideMenuVO::getName).collect(Collectors.toList());
      // 添加新增的数据
      List<ChatSideMenuVO> newMenu =
          initMenu.stream()
      ...
      for (ChatSideMenuVO menu : result) {
        initMenu.stream()
            ...
            .findAny()
            .ifPresent(im -> menu.setType(im.getType()));
      }
    } else {
      // 初始化
      result =
