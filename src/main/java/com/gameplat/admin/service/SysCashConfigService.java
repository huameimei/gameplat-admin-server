...
import com.gameplat.model.entity.sys.SysCashConfig;
...
public class SysCashConfigService {
          ...
          a -> {
            ...
            if ("oddDaysCashTriesLimit".equals(a.getDictLabel())) {
              if (finalWithdrawLimit > 0) {
                sysCashConfig.setWithdrawCount(
                    new BigDecimal(finalWithdrawLimit)
                        .subtract(new BigDecimal(s.get("num").toString())));
              } else {
                sysCashConfig.setWithdrawCount(
                    new BigDecimal(a.getDictValue())
                        .subtract(new BigDecimal(s.get("num").toString())));
              }
            }
            ...
            if ("oddDaysCashMoneyUpperLimit".equals(a.getDictLabel())) {
              ...
              sysCashConfig.setWithdrawtRanslateAmount(
                  bigDecimal.subtract(new BigDecimal(s.get("amount").toString())));
            }
  ...
  public int getWithdrawLimit(Long userId) {
    ...
    try {
      // 根据用户id查询用户当前的等级
      // 查询当前的用户的汇总的充值成长值
      MemberGrowupTotalrecord memberGrowUTotal =
          growupTotalrecordMapper.searchGrowUpToalByMemberId(userId);
