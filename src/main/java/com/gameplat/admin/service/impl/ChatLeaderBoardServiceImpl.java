          ...
          a -> {
            if (((JSONObject) a).getIntValue("isOpenLeaderboard") != 0) {
              ...
              if (StringUtil.isNotBlank(leaderBoard)) {
                List<ChatLeaderBoardVO> leaderBoardVOS =
                    JSON.parseArray(leaderBoard, ChatLeaderBoardVO.class);
              ...
              // 添加自定义推送中奖信息的会员数据
              List<ChatLeaderBoardVO> pushWinList =
                  chatLeaderBoardMapper.slaveQueryPushWinReport(id, gameIds, statTime);
              ...
              if (chatPushCPBet != null) {
                ...
                if (StringUtils.isNotBlank(leaderBoardBlackAccount)) {
                  ...
                  if (blackAccount.size() > 0) {
                    result =
                        result.stream()
                            .filter(r -> !blackAccount.contains(r.getAccount()))
              ...
              if (result.size() > 0) {
                ...
                List<String> userIds =
                    result.stream()
                        .map(ChatLeaderBoardVO::getUserId)
                        .map(Object::toString)
                        .collect(Collectors.toList());
                try {
                          ...
                          userList.forEach(
                              u -> {
                                ...
                                if (c.getUserId().equals(userId)) {
                                  ...
                                  c.setWinMoney(
                                      Double.valueOf(String.format("%.2f", c.getWinMoney())));
              ...
              redisTemplate
                  .opsForValue()
                  .set(
                      String.format("%s_%s_%s", proxy, id.toString(), CHAT_LEADER_BOARD),
                      JSON.toJSONString(chatLeaderBoardVOS),
                      2,
                      TimeUnit.MINUTES);
      ...
      if (code == 200) {
        ...
        if (!chatLeaderBoardVOS.isEmpty()) {
          ...
          List<String> accounts =
              chatLeaderBoardVOS.stream()
                  .map(ChatLeaderBoardVO::getAccount)
                  .collect(Collectors.toList());
