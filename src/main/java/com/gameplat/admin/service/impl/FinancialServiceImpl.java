...
public class FinancialServiceImpl extends ServiceImpl<FinancialMapper, Financial>
    implements FinancialService {
  ...
  public int insertFinancial(Financial financial) {
    ...
    if (financial.getCurrencyType() == CurrencyTypeEnum.REAL.getCode()) {
      ...
      if (sysCurrencyRate == null
          || sysCurrencyRate.getRechargeAndroidRate() == null
          || sysCurrencyRate.getRechargeAndroidRate().compareTo(BigDecimal.ZERO) <= 0) {
        throw new ServiceException("操作失败！");
      }
      financial.setAmountCny(
          financial
              .getAmount()
              .divide(sysCurrencyRate.getRechargeAndroidRate(), 2, BigDecimal.ROUND_HALF_UP));
    }
    ...
    if (financial.getCurrencyType() == CurrencyTypeEnum.TOKEN.getCode()
      ...
      if (sysCashConfigVO.getRate() == null
          || sysCashConfigVO.getRate().compareTo(BigDecimal.ZERO) <= 0) {
        throw new ServiceException("操作失败！");
      }
