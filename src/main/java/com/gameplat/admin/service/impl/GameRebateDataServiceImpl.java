...
public class GameRebateDataServiceImpl extends ServiceImpl<GameRebateDataMapper, GameRebateData>
  ...
  public void saveRebateDayReport(String statTime, GamePlatform gamePlatform) {
    log.info(
        "{}[{}],statTime:[{}]> Start save game_rebate_data",
    ...
    builder.filter(
        QueryBuilders.rangeQuery("statTime.keyword")
            .from(beginTime)
            .to(endTime)
    ...
    try {
      ...
      if (count > 0) {
        log.info(
            "{}[{}],statTime:[{}]> game_rebate_data Rebate bet record data size:[{}]",
            gamePlatform.getName(),
            gamePlatform.getCode(),
            statTime,
            count);
        ...
        updateWrapper
            .eq(GameRebateData::getPlatformCode, gamePlatform.getCode())
            .eq(GameRebateData::getStatTime, statTime);
        int deleted = gameRebateDataMapper.delete(updateWrapper);
        log.info(
            "{}[{}],statTime:[{}]> game_rebate_data deleted data size:[{}]",
            gamePlatform.getName(),
            gamePlatform.getCode(),
            statTime,
            deleted);
        ...
        log.info(
            "{}[{}],statTime:[{}]> no data save to game_rebate_data",
            gamePlatform.getName(),
            gamePlatform.getCode(),
            statTime);
      }
      log.info(
          "{}[{}],statTime:[{}]> End  save game_rebate_data",
          gamePlatform.getName(),
          gamePlatform.getCode(),
          statTime);
    } catch (IOException e) {
