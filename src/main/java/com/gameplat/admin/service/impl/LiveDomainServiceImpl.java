...
public class LiveDomainServiceImpl implements ILiveDomainService {
  ...
  @Autowired
  private SportProperty sportProperty;

  @Autowired
  private SportRequestUtil requestUtil;
  ...
  public String getLiveDomainTrafficData(LiveDomainParamDTO param) {
    try {
      ...
      SportGameConfig config = JSONObject.parseObject(sportConfigJson.toJSONString(),SportGameConfig.class);
      ...
      String result = HttpRequest.post(url).header("tenant",tenant).header("country",param.getCountry() == null ? "zh-CN" : param.getCountry())
