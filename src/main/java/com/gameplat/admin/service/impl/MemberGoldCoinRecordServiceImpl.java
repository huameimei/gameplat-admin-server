...
public class MemberGoldCoinRecordServiceImpl
  ...
  public void importAddGoldCoin(MultipartFile file) throws IOException {
    ...
    for (int i = 0; i < list.size(); i++) {
      try {
        ...
        MemberGoldCoinRecord userGoldCoinRecord =
            MemberGoldCoinRecord.builder()
        ...
        wrapper
            .set(MemberInfo::getGoldCoin, memberGoldCoin + amount)
  ...
  public void addGoldCoin(String[] accountArray, Integer amount) {
    for (String account : accountArray) {
      Member member =
          memberService.getByAccount(account).orElseThrow(() -> new ServiceException("账号不存在"));
      MemberInfo memberInfo = memberInfoService.getById(member.getId());
      ...
      String remark = "后台添加金币，用户：" + member.getAccount() + ",金币：" + amount;
      MemberGoldCoinRecord memberGoldCoinRecord =
          MemberGoldCoinRecord.builder()
              .memberId(member.getId())
              .account(member.getAccount())
              .sourceType(3)
      ...
      LambdaUpdateWrapper<MemberInfo> wrapper = new LambdaUpdateWrapper<>();
      wrapper
          .set(MemberInfo::getGoldCoin, userGoldCoin + amount)
          .eq(MemberInfo::getMemberId, member.getId());
      memberInfoService.update(wrapper);
    }
  }
