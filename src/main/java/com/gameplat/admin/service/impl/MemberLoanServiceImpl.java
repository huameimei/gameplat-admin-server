...
public class MemberLoanServiceImpl extends ServiceImpl<MemberLoanMapper, MemberLoan>
  ...
  public LoanVO page(PageDTO<MemberLoan> page, MemberLoanQueryDTO dto) {
    // 获取符合资格的会员
    List<MemberGrowthLevel> memberGrowthLevelList =
        memberGrowthLevelService.lambdaQuery().gt(MemberGrowthLevel::getLoanMoney, 0.00).list();
    ...
    for (MemberGrowthLevel memberGrowthLevel : memberGrowthLevelList) {
      ...
      memberList.addAll(memberVoList);

      for (MemberVO memberVO : memberList) {
        // 已经有数据就不添加
        MemberLoan memberLoan =
            this.lambdaQuery().eq(MemberLoan::getMemberId, memberVO.getId()).one();

        if (ObjectUtil.isNull(memberLoan)) {
          MemberLoan saveMemberLoan =
              new MemberLoan()
                  ...
                  .setLoanStatus(1)
                  .setOverdraftMoney(BigDecimal.ZERO);
          this.save(saveMemberLoan);
        }
      }
    }

    IPage<MemberLoanVO> pageList =
        this.lambdaQuery()
            ...
            .ge(
                ObjectUtil.isNotNull(dto.getMinOverdraftMoney()),
                MemberLoan::getOverdraftMoney,
                dto.getMinOverdraftMoney())
            .le(
                ObjectUtil.isNotNull(dto.getMaxOverdraftMoney()),
                MemberLoan::getOverdraftMoney,
                dto.getMaxOverdraftMoney())
            .ge(
                ObjectUtil.isNotNull(dto.getBeginLoanTime()),
                MemberLoan::getLoanTime,
                dto.getBeginLoanTime())
            .le(
                ObjectUtil.isNotNull(dto.getEndLoanTime()),
                MemberLoan::getLoanTime,
                dto.getEndLoanTime())
            .eq(
                ObjectUtil.isNotNull(dto.getLoanStatus()),
                MemberLoan::getLoanStatus,
                dto.getLoanStatus())
            .page(page)
  ...
  public void editOrUpdate(MemberLoan memberLoan) {
    MemberLoan loan =
        this.lambdaQuery().eq(MemberLoan::getMemberId, memberLoan.getMemberId()).one();
    if (ObjectUtil.isNotNull(loan)) {
      LambdaUpdateWrapper<MemberLoan> wrapper = new LambdaUpdateWrapper<>();
      wrapper
          .set(MemberLoan::getLoanMoney, memberLoan.getLoanMoney())
  ...
  public void recycle(String idList) {
    ...
    for (String loanId : split) {
      ...
      MemberLoan memberLoan = this.lambdaQuery().eq(MemberLoan::getId, id).one();
      if (ObjectUtil.isNull(memberLoan)) {
