...
public class SpreadUnionServiceImpl extends ServiceImpl<SpreadUnionMapper, SpreadUnion>
   ...
   * 获取联盟报表
   *
   * @return 一级 todo 日期 联盟名称 联运类型 代理账号 联运平台 联运收益 充值金额 提现金额 注册人数 更新时间
   */
  @Override
  public Object getUnionReportList(SpreadUnionDTO dto) {
    ...
    List<SpreadUnionVO> list =
        this.lambdaQuery()
            .eq(SpreadUnion::getAgentAccount, dto.getAgentAccount())
            .like(SpreadUnion::getUnionName, dto.getUnionName())
            .list()
            .stream()
    ...
    //
    //        //会员表。获取代理线下的所有会员
    //        List<Member> agentMember = memberMapper.getAgentMember(list,dto.getStartTime(),
    // dto.getEndTime());
    //        //会员记录表，获取在相关时间段内有进行过转代理等操作的相关代理线下的会员
    //        List<String> content = memberBackupService.getContent(null, dto.getStartTime(),
    // dto.getEndTime());
    //        //取出满足条件的会员
